...
export class DataStorage {
  private onelakeConnectionString: string;
  ...
  constructor() {
    ...
    if (!this.onelakeConnectionString) {
      logger.warn('ONELAKE_CONNECTION_STRING not provided, using mock storage');
    }
  }
   ...
   */
  async storeInvestigationData(data: WalletInvestigationData): Promise<void> {
    try {
      logger.info(`Storing investigation data for wallet: ${data.walletAddress}`);
      ...
      logger.error('Failed to store investigation data:', error);
      throw error;
    }
  }
   ...
   */
  private async storeToOneLake(data: WalletInvestigationData): Promise<void> {
    try {
      ...
      logger.info(`Storing to OneLake: ${storageData.id}`);
      ...
      logger.error('OneLake storage failed:', error);
      throw error;
    }
  }
   ...
   */
  private async storeToMockStorage(data: WalletInvestigationData): Promise<void> {
    try {
      const mockData = {
        id: `${data.walletAddress}-${data.investigationTimestamp.getTime()}`,
        walletAddress: data.walletAddress,
   ...
   */
  async getInvestigationData(walletAddress: string, blockchain?: string): Promise<WalletInvestigationData[]> {
    try {
      logger.info(`Retrieving investigation data for wallet: ${walletAddress}`);
      ...
      logger.error('Failed to retrieve investigation data:', error);
      throw error;
    }
  }
   ...
   */
  private async getFromOneLake(walletAddress: string, blockchain?: string): Promise<WalletInvestigationData[]> {
    try {
      // This would implement actual OneLake retrieval
      // For now, returning empty array
      
      logger.info(`Retrieving from OneLake for wallet: ${walletAddress}`);
      ...
      logger.error('OneLake retrieval failed:', error);
      throw error;
    }
  }
   ...
   */
  private async getFromMockStorage(walletAddress: string, blockchain?: string): Promise<WalletInvestigationData[]> {
    try {
      logger.info(`Retrieving from mock storage for wallet: ${walletAddress}`);
      ...
      logger.error('Mock storage retrieval failed:', error);
      throw error;
    }
  }
  ...
  async getInvestigationStats(): Promise<{
    ...
    try {
      logger.info('Retrieving investigation statistics');
      ...
      logger.error('Failed to retrieve investigation statistics:', error);
      throw error;
    }
  }
   ...
   */
  private async getStatsFromOneLake(): Promise<{
    totalInvestigations: number;
    investigationsByBlockchain: Record<string, number>;
    recentInvestigations: number;
    averageProcessingTime: number;
    ...
    try {
      ...
      logger.error('OneLake statistics retrieval failed:', error);
      throw error;
    }
  }
   ...
   */
  private async getStatsFromMockStorage(): Promise<{
    totalInvestigations: number;
    investigationsByBlockchain: Record<string, number>;
    recentInvestigations: number;
    averageProcessingTime: number;
    ...
    try {
      ...
      logger.error('Mock storage statistics retrieval failed:', error);
      throw error;
    }
  }
  ...
  async cleanupOldData(retentionDays: number = 365): Promise<void> {
    try {
      logger.info(`Cleaning up investigation data older than ${retentionDays} days`);
   ...
   */
  private async cleanupOneLakeData(retentionDays: number): Promise<void> {
    try {
      // This would implement actual OneLake cleanup
      // For now, just logging the action
      
      logger.info(`Cleaning up OneLake data older than ${retentionDays} days`);
      ...
      logger.error('OneLake cleanup failed:', error);
      throw error;
    }
  }
   ...
   */
  private async cleanupMockStorageData(retentionDays: number): Promise<void> {
    try {
      // This would implement actual mock storage cleanup
      // For now, just logging the action
      
      logger.info(`Cleaning up mock storage data older than ${retentionDays} days`);
      
    } catch (error) {
      logger.error('Mock storage cleanup failed:', error);
      throw error;
    }
  }
   ...
   */
  async storeAgentMessage(message: any): Promise<void> {
    try {
      logger.info(`Storing agent message from ${message.sender} to ${message.recipient}`);
      ...
      logger.error('Failed to store agent message:', error);
      throw error;
    }
  }
   ...
   */
  private async storeAgentMessageToOneLake(message: any): Promise<void> {
    try {
      const storageData = {
        id: message.id,
      ...
      logger.info(`Storing agent message to OneLake: ${storageData.id}`);
      
      // In a real implementation, this would use the OneLake SDK
      // await onelakeClient.container(this.containerName).item(storageData.id).create(storageData);
      
    } catch (error) {
      logger.error('OneLake agent message storage failed:', error);
      throw error;
    }
  }
   ...
   */
  private async storeAgentMessageToMockStorage(message: any): Promise<void> {
    try {
      const mockData = {
        id: message.id,
        timestamp: message.timestamp,
      ...
      logger.info(`Stored agent message to mock storage: ${mockData.id}`);
      
      // In a real implementation, this would store to a local database or file
      // For now, just logging the storage action
      
    } catch (error) {
      logger.error('Mock storage agent message failed:', error);
      throw error;
    }
  }
} 
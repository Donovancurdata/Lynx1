...
export class TransactionAnalyzer {
   ...
   */
  async analyzeTransactions(transactions: Transaction[], blockchain: string): Promise<TransactionAnalysis> {
    try {
      logger.info(`Analyzing ${transactions.length} transactions for blockchain: ${blockchain}`);
   ...
   */
  private analyzeTransactionTypes(transactions: Transaction[]): Record<string, number> {
   ...
   */
  private analyzeTemporalPatterns(transactions: Transaction[]): {
   ...
   */
  private analyzeValueDistribution(transactions: Transaction[]): {
    totalValue: number;
    averageValue: number;
    medianValue: number;
    minValue: number;
    ...
    values.forEach(value => {
      if (value <= 0.001) valueRanges['0-0.001']++;
      else if (value <= 0.01) valueRanges['0.001-0.01']++;
      else if (value <= 0.1) valueRanges['0.01-0.1']++;
      else if (value <= 1) valueRanges['0.1-1']++;
      else if (value <= 10) valueRanges['1-10']++;
      else if (value <= 100) valueRanges['10-100']++;
      else valueRanges['100+']++;
    });
  ...
  private analyzeCounterparties(transactions: Transaction[]): {
    ...
    topCounterparties: Array<{ address: string; interactionCount: number; totalValue: number }>;
   ...
   */
  private analyzeGasUsage(transactions: Transaction[]): {
    totalGasUsed: number;
    ...
    const highGasTransactions = gasTransactions.filter(tx => 
      (parseInt(tx.gasUsed || '0') || 0) > 100000
    ).length;
   ...
   */
  private analyzeRiskPatterns(transactions: Transaction[]): {
   ...
   */
  private calculateTotalDays(transactions: Transaction[]): number {

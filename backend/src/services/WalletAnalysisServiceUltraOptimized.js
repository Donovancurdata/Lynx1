...
exports.WalletAnalysisServiceUltraOptimized = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
const path_1 = __importDefault(require("path"));
// Load environment variables
dotenv_1.default.config({ path: path_1.default.resolve(process.cwd(), '.env') });
class WalletAnalysisServiceUltraOptimized {
    static detectAllBlockchains(address) {
        const blockchains = [];
    ...
    static async analyzeWallet(address, deepAnalysis = false) {
    ...
    static async analyzeSingleBlockchain(address, blockchain, _deepAnalysis) {
        const analysis = {
            address,
            blockchain,
    ...
    static async getUltraOptimizedBlockchainData(address, blockchain) {
        const config = this.BLOCKCHAIN_APIS[blockchain];
        ...
        try {
            ...
            if (txData.status === '1' && txData.result) {
                ...
                const mappedTransactions = txData.result.slice(0, 3).map((tx) => ({
                    hash: tx.hash,
                    from: tx.from,
                    to: tx.to,
    ...
    static async getBalanceViaRPC(address, blockchain, balance) {
        const rpcUrls = {
    ...
    static async getCachedTokenPrice(symbol) {
        const now = Date.now();
    ...
    static compileResults(address, results) {
...
exports.WalletAnalysisServiceUltraOptimized = WalletAnalysisServiceUltraOptimized;
// Caches for performance optimization
WalletAnalysisServiceUltraOptimized.analysisCache = new Map();
WalletAnalysisServiceUltraOptimized.tokenPriceCache = new Map();
WalletAnalysisServiceUltraOptimized.CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
WalletAnalysisServiceUltraOptimized.MAX_TRANSACTIONS_PER_CHAIN = 5; // Reduced for speed
WalletAnalysisServiceUltraOptimized.MAX_TOKENS_PER_CHAIN = 5; // Reduced for speed
// Blockchain-specific API configurations using Etherscan multi-chain API
WalletAnalysisServiceUltraOptimized.BLOCKCHAIN_APIS = {
    ethereum: {
...
//# sourceMappingURL=WalletAnalysisServiceUltraOptimized.js.map